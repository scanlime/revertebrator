...
```
usage: rvtool [-h] [-F FILE] {scan,forget,list,pack} ...

positional arguments:
  {scan,forget,list,pack}
...
options:
  ...
  --block-size SEC     length of audio blocks to process, in seconds [8]
...
options:
  ...
  -s SQL      comma-separated list of expressions to sort files by [path]
  -f SQL      additional expression to filter results with [1]
  -o DEST     name of packed output file [voice-20220522170746.rvv]
  -w S        maximum grain width in seconds [3.0]
  -n N        maximum number of grains per bin [500]

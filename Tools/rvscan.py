...
parser.add_argument('-P', metavar='N', dest='parallelism', type=int,
                    help='number of parallel jobs to run [#CPUs]')
parser.add_argument('-n', metavar='N', dest='file_limit', type=int,
...
def load_one(p):
    ...
    f0, voiced_flag, voiced_probs = librosa.pyin(
        y, sr=args.sr, fmin=args.fmin, fmax=args.fmax, fill_na=None,
        resolution=args.res)
...
for srcdir in args.inputs:
    paths.extend(os.path.join(srcdir, d)
...
np.savez(grain_filename, y=samples, f0=grain_f0, x=grain_x, ylen=len(samples),
         sr=args.sr, vprob=args.vprob, fmin=args.fmin, fmax=args.fmax)
